...
import { users, processedRiotMatches, riotAccounts } from '@shared/schema';
...
function generateToken(userId, email) {
    return jwt.sign({ userId, email, source: 'desktop' }, JWT_SECRET, { expiresIn: '7d' });
}
/**
 * Verify JWT token
 */
 ...
 */
desktopApiRouter.post('/verify-match', requireDesktopAuth, async (req, res) => {
    try {
        ...
        // Award points
        await pointsEngine.awardPoints(userId, pointsToAward, 'desktop_verification', matchKey, 'desktop_app', `${gameName} session - ${Math.round(sessionDuration / 60)} min`);
        // Record as processed
        if (riotAccount) {
 ...
 */
desktopApiRouter.get('/balance', requireDesktopAuth, async (req, res) => {
    try {
        const { userId } = req.desktopUser;
        const [user] = await db.select()
            .from(users)
