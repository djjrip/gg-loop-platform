...
router.get('/trends', isAuthenticated, adminOnly, async (req, res) => {
    try {
        ...
        END)`
            ...
            .from(subscriptions)
            .where(and(eq(subscriptions.status, 'active'), gte(subscriptions.createdAt, startDate)))
        ...
        const redemptionRevenue = await db
            .select({
            date: sql `DATE(${userRewards.redeemedAt})`,
            revenue: sql `SUM(${rewards.realValue}) / 100.0`
        })
            .from(userRewards)
            .innerJoin(rewards, eq(userRewards.rewardId, rewards.id))
            .where(and(eq(userRewards.status, 'fulfilled'), gte(userRewards.redeemedAt, startDate)))
...
router.get('/top-rewards', isAuthenticated, adminOnly, async (req, res) => {
    try {
        const topRewards = await db
            ...
            totalValue: sql `SUM(${rewards.realValue}) / 100.0`,
            totalCommissions: sql `SUM(${rewards.affiliateCommission || 0}) / 100.0`
        })
            .from(userRewards)
            .innerJoin(rewards, eq(userRewards.rewardId, rewards.id))
            .where(eq(userRewards.status, 'fulfilled'))
