...
const getPlanIdForTier = (tier) => {
    ...
    if (isProduction) {
        return PROD_PLANS[tier];
    }
    return SANDBOX_PLANS[tier];
};
...
router.post("/subscription-approved", async (req, res) => {
    try {
        ...
        const points = tierPoints[verification.tier] || 0;
        if (points > 0) {
            await pointsEngine.awardPoints(userId, points, 'MONTHLY_ALLOWANCE', subscriptionId, 'subscription', `Monthly points for ${verification.tier} tier`);
        }
        res.json({
            success: true,
                ...
                if (billingAgreementId) {
                    ...
                    if (existingSub && existingSub.status === 'active') {
                        ...
                        if (points > 0) {
                            ...
                            await pointsEngine.awardPoints(existingSub.userId, points, 'MONTHLY_ALLOWANCE', transactionId || billingAgreementId, 'subscription', `Monthly renewal points for ${existingSub.tier} tier`);
