...
import { users, auditLog, rewardClaims, rewardTypes, insertRewardTypeSchema, updateRewardClaimSchema } from "@shared/schema";
...
router.post("/fulfillment/reward-types", async (req, res) => {
    try {
        ...
        await fulfillmentService.logAction(adminUser.id, adminUser.email || "unknown", "REWARD_TYPE_CREATED", adminUser.id, { rewardTypeId: result[0].id, name: parsed.name });
        res.json(result[0]);
    }
    catch (error) {
...
router.get("/fulfillment/claims", async (req, res) => {
    try {
        ...
        const { claims, total } = await fulfillmentService.getClaimsFiltered({ status, userId }, limit, offset);
...
router.patch("/fulfillment/claims/:id", async (req, res) => {
    try {
        ...
        await fulfillmentService.logAction(adminUser.id, adminUser.email || "unknown", actionType, updatedClaim.userId, {
            claimId,
