...
import { rewards, userRewards } from "@shared/schema";
 ...
 */
rewardRedemptionRouter.get("/", async (req, res) => {
    try {
 ...
 */
rewardRedemptionRouter.post("/redeem", async (req, res) => {
    try {
        ...
        // 5. Deduct points
        const transaction = await pointsEngine.spendPoints(userId, reward.pointsCost, "reward_redemption", rewardId, "reward", `Redeemed: ${reward.title}`);
 ...
 */
rewardRedemptionRouter.get("/my-redemptions", async (req, res) => {
    try {
        const user = req.user || req.dbUser;
        if (!user?.id) {
 ...
 */
rewardRedemptionRouter.get("/catalog", async (req, res) => {
    try {
        const availableRewards = await db.select()
            .from(rewards)
            .where(eq(rewards.inStock, true));
