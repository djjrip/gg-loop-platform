 ...
 */
matchSyncRouter.post("/sync-matches", async (req, res) => {
    try {
        ...
        const recentWins = await riotApi.getRecentWins(riotAccount.puuid, riotAccount.region || 'americas', 24 // hours
        );
        ...
        for (const win of recentWins) {
            ...
            const [existing] = await db.select()
                ...
                .where(and(eq(processedRiotMatches.matchId, win.matchId), eq(processedRiotMatches.riotAccountId, riotAccount.id)));
            if (!existing) {
                newWins.push(win);
            }
        }
        ...
        for (const win of newWins) {
            try {
                ...
                // Award points
                const transaction = await pointsEngine.awardPoints(userId, basePoints, "match_win", win.matchId, "riot_lol_match", `${win.championName} - ${win.kills}/${win.deaths}/${win.assists}`);
 ...
 */
matchSyncRouter.get("/sync-status", async (req, res) => {
    try {
        const user = req.user || req.dbUser;
        if (!user?.id) {
