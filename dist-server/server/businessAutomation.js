...
import { rewardClaims, users, subscriptions, pointTransactions, rewardTypes } from '@shared/schema';
 ...
 */
export async function runBusinessAutomation() {
...
async function analyzeBusinessHealth() {
    ...
    const needsReview = pendingClaims.filter((claim) => {
    ...
    return {
        ...
        redemptions: {
            ...
            totalValue: pendingClaims.reduce((sum, claim) => {
                const rewardType = rewardTypeMap.get(claim.rewardTypeId);
 ...
 */
async function autoApproveSafeRedemptions(health) {
    const pendingClaims = await db
        .select()
        .from(rewardClaims)
        .where(eq(rewardClaims.status, 'pending'))
    ...
    for (const claim of pendingClaims) {
        ...
        if (rewardType.realValue <= 5000) { // $50 in cents
            ...
            const userClaimsResult = await db
                .select()
                .from(rewardClaims)
                .where(and(eq(rewardClaims.userId, claim.userId), eq(rewardClaims.status, 'fulfilled')));
 ...
 */
async function handleCommonSupportIssues() {
 ...
 */
async function autoOptimizePricing(health) {
 ...
 */
async function sendDailyBusinessReport(health) {
