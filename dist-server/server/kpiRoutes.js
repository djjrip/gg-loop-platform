...
router.get("/api/admin/kpi-dashboard", async (req, res) => {
    try {
        // Admin check
        if (!req.isAuthenticated() || !req.user?.oidcSub) {
            return res.status(401).json({ message: "Unauthorized" });
        ...
        const [newUsers24h] = await db.select({ count: sql `count(*)::int` }).from(users).where(gte(users.createdAt, last24h));
        const [newUsers7d] = await db.select({ count: sql `count(*)::int` }).from(users).where(gte(users.createdAt, last7d));
        const [activeUsers] = await db.select({ count: sql `count(distinct ${users.id})::int` }).from(users)
            .innerJoin(pointTransactions, eq(users.id, pointTransactions.userId))
            .where(gte(pointTransactions.createdAt, last7d));
        // Revenue Metrics
        const [totalRevenue] = await db.select({ sum: sql `COALESCE(SUM(amount), 0)::float` }).from(subscriptions);
        const [revenue24h] = await db.select({ sum: sql `COALESCE(SUM(amount), 0)::float` }).from(subscriptions).where(gte(subscriptions.createdAt, last24h));
        const [revenue7d] = await db.select({ sum: sql `COALESCE(SUM(amount), 0)::float` }).from(subscriptions).where(gte(subscriptions.createdAt, last7d));
        const [revenue30d] = await db.select({ sum: sql `COALESCE(SUM(amount), 0)::float` }).from(subscriptions).where(gte(subscriptions.createdAt, last30d));
        // Points Economy
        const [totalPointsIssued] = await db.select({ sum: sql `COALESCE(SUM(amount), 0)::int` }).from(pointTransactions).where(sql `amount > 0`);
        const [totalPointsSpent] = await db.select({ sum: sql `COALESCE(SUM(ABS(amount)), 0)::int` }).from(pointTransactions).where(sql `amount < 0`);
        const [pointsIssued24h] = await db.select({ sum: sql `COALESCE(SUM(amount), 0)::int` }).from(pointTransactions).where(and(sql `amount > 0`, gte(pointTransactions.createdAt, last24h)));
        // Engagement Metrics
        const [totalRedemptions] = await db.select({ count: sql `count(*)::int` }).from(userRewards);
        const [redemptions24h] = await db.select({ count: sql `count(*)::int` }).from(userRewards).where(gte(userRewards.redeemedAt, last24h));
        const [totalChallenges] = await db.select({ count: sql `count(*)::int` }).from(challenges);
        const [completedChallenges] = await db.select({ count: sql `count(*)::int` }).from(challengeCompletions);
        // Top Users by Points
