...
import { rewardClaims, rewardTypes, auditLog, users, } from "@shared/schema";
...
export class FulfillmentService {
     ...
     */
    async createRewardClaim(userId, rewardTypeId, ipAddress, userAgent) {
     ...
     */
    async updateClaimStatus(claimId, adminUserId, update) {
     ...
     */
    async getMissionControlMetrics() {
        // Total counts
        const claimStats = await db
            .select({
            total: sql `count(*)`,
            pending: sql `sum(case when status = 'pending' then 1 else 0 end)`,
            fulfilled: sql `sum(case when status = 'fulfilled' then 1 else 0 end)`,
            rejected: sql `sum(case when status = 'rejected' then 1 else 0 end)`,
            totalUsdSpent: sql `coalesce(sum(case when status = 'fulfilled' then rt.real_value else 0 end), 0)`,
        })
            .from(rewardClaims)
        ...
        const fulfillmentRate = totalClaims > 0 ? Math.round((fulfilledClaims / totalClaims) * 100) : 0;
        // Top reward types
        const topTypes = await db
            .select({
            type: rewardTypes.type,
            count: sql `count(${rewardClaims.id})`,
        })
            .from(rewardClaims)
            .leftJoin(rewardTypes, eq(rewardClaims.rewardTypeId, rewardTypes.id))
        ...
        const streamerMetrics = await db
            ...
            claimCount: sql `count(${rewardClaims.id})`,
        })
            .from(rewardClaims)
     ...
     */
    async getUserClaimMetrics(userId) {
        const userResult = await db.select().from(users).where(eq(users.id, userId));
     ...
     */
    async getPendingClaims(limit = 50, offset = 0) {
     ...
     */
    async getClaimsFiltered(filters, limit = 50, offset = 0) {
     ...
     */
    async logAction(adminUserId, adminEmail, action, targetUserId, details, ipAddress) {
