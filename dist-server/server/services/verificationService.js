 ...
 */
export async function verifyMatchSubmission(matchSubmissionId) {
    ...
    const proofs = await db
        ...
        .where(and(eq(verificationProofs.sourceType, 'match_submission'), eq(verificationProofs.sourceId, matchSubmissionId)));
    if (proofs.length > 0) {
        // Has proof - add score
...
export async function enqueueVerificationItem(params) {
    // Determine priority
    let priority = params.priority || 1;
    // Check for existing queue item
    const existing = await db
        .select()
        .from(verificationQueue)
        .where(and(eq(verificationQueue.itemType, params.itemType), eq(verificationQueue.itemId, params.itemId)))
        .limit(1);
    ...
    const slaHours = verificationConfig.queue.sla[priority === 4 ? 'critical' :
        priority === 3 ? 'high' :
...
export async function getVerificationStatusForUser(userId) {
    ...
    const pendingItems = await db
        .select()
        .from(verificationQueue)
        .where(and(eq(verificationQueue.userId, userId), eq(verificationQueue.status, 'pending')));
    ...
    submissions.forEach(s => {
        if (s.fraudFlags && Array.isArray(s.fraudFlags)) {
            activeFraudFlags.push(...s.fraudFlags);
        }
    });
    return {
...
export async function processAdminReview(params) {
    ...
    if (queueItem.itemType === 'match_submission') {
        ...
        if (newStatus === 'approved') {
            await TrustScoreService.logEvent(queueItem.userId, 'MATCH_VERIFIED_MANUAL', 1, // Value depends on scoring logic, but logEvent will trigger recalc
            `Match ${queueItem.itemId} manually verified by admin`, 'admin_review');
        }
    }
        ...
        if (newStatus === 'verified') {
            await TrustScoreService.logEvent(queueItem.userId, 'PROOF_VERIFIED_MANUAL', 10, // Proofs usually worth more
            `Proof ${queueItem.itemId} manually verified by admin`, 'admin_review');
        }
    }
    ...
    if (params.action === 'flag' && params.fraudFlags) {
        ...
        // Log trust penalty
        await TrustScoreService.logEvent(queueItem.userId, 'FRAUD_FLAG_MANUAL', -50, `Flagged by admin: ${params.notes}`, 'admin_review');
    }
    return { success: true };
}
