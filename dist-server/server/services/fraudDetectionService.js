 ...
 */
async function checkDuplicateSubmissions(userId, currentSubmissionId) {
    ...
    const recentSubmissions = await db
        ...
        .where(and(eq(matchSubmissions.userId, userId), gte(matchSubmissions.createdAt, windowStart)));
 ...
 */
async function checkImpossibleTiming(userId) {
    const oneHourAgo = new Date(Date.now() - 60 * 60 * 1000);
    const maxMatchesPerHour = verificationConfig.fraud.sensitivity.impossibleTiming.maxMatchesPerHour;
    const recentMatches = await db
        .select()
        .from(matchSubmissions)
        .where(and(eq(matchSubmissions.userId, userId), gte(matchSubmissions.createdAt, oneHourAgo)));
 ...
 */
async function checkPatternAnomaly(userId, matchData) {
 ...
 */
async function checkRapidProgression(userId) {
    const oneDayAgo = new Date(Date.now() - 24 * 60 * 60 * 1000);
    const recentMatches = await db
        .select()
        .from(matchSubmissions)
        .where(and(eq(matchSubmissions.userId, userId), gte(matchSubmissions.createdAt, oneDayAgo)));
    const totalPoints = recentMatches
        .filter(m => m.pointsAwarded !== null)
        .reduce((sum, m) => sum + m.pointsAwarded, 0);
...
export async function getActiveFraudAlerts(filters) {
    ...
    if (filters?.severity) {
        query = query.where(and(eq(fraudDetectionLogs.status, 'pending'), eq(fraudDetectionLogs.severity, filters.severity)));
    }
    if (filters?.userId) {
        query = query.where(and(eq(fraudDetectionLogs.status, 'pending'), eq(fraudDetectionLogs.userId, filters.userId)));
    }
    if (filters?.limit) {
