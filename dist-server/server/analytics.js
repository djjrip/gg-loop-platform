...
export async function getPlatformOverview() {
    return await getCached(CacheKeys.platformOverview(), async () => {
        const now = new Date();
        ...
        const totalUsersResult = await db
            .select({ count: count() })
            .from(users)
            .where(and(sql `${users.fraudScore} <= 30`, eq(users.banned, false)));
        ...
        const totalXPResult = await db
            ...
            .where(and(eq(pointTransactions.verified, true), sql `${users.fraudScore} <= 30`, eq(users.banned, false)));
        const totalXP = totalXPResult[0]?.total || 0;
        // New users 24h
        const newUsers24h = await db
            .select({ count: count() })
            .from(users)
            .where(and(gte(users.createdAt, day24Ago), sql `${users.fraudScore} <= 30`, eq(users.banned, false)));
        // Active users 24h
        const activeUsers24h = await db
            .select({ count: sql `COUNT(DISTINCT ${pointTransactions.userId})` })
            .from(pointTransactions)
            .leftJoin(users, eq(pointTransactions.userId, users.id))
            .where(and(gte(pointTransactions.createdAt, day24Ago), sql `${users.fraudScore} <= 30`, eq(users.banned, false)));
        return {
            users: {
                totalUsers,
...
export async function getUserAnalytics(userId) {
    return await getCached(CacheKeys.userAnalytics(userId), async () => {
        const user = await db.query.users.findFirst({
        ...
        const totalXPResult = await db
            .select({ total: sql `COALESCE(SUM(${pointTransactions.amount}), 0)` })
            .from(pointTransactions)
            .where(and(eq(pointTransactions.userId, userId), eq(pointTransactions.verified, true)));
        return {
...
export async function getEngagementMetrics() {
    return await getCached(CacheKeys.engagementMetrics(), async () => {
        const now = new Date();
        ...
        const dauResult = await db
            ...
            .from(pointTransactions)
            .leftJoin(users, eq(pointTransactions.userId, users.id))
            .where(and(gte(pointTransactions.createdAt, day1Ago), sql `${users.fraudScore} <= 30`, eq(users.banned, false)));
        const mauResult = await db
            .select({ count: sql `COUNT(DISTINCT ${pointTransactions.userId})` })
            .from(pointTransactions)
            .leftJoin(users, eq(pointTransactions.userId, users.id))
            .where(and(gte(pointTransactions.createdAt, day30Ago), sql `${users.fraudScore} <= 30`, eq(users.banned, false)));
...
export async function getRevenueMetrics() {
    return await getCached(CacheKeys.revenueMetrics(), async () => {
        const now = new Date();
        const day1Ago = new Date(now.getTime() - 24 * 60 * 60 * 1000);
        const totalXPResult = await db
            .select({ total: sql `COALESCE(SUM(${pointTransactions.amount}), 0)` })
        ...
        const xpLast24hResult = await db
            ...
            .from(pointTransactions)
            .leftJoin(users, eq(pointTransactions.userId, users.id))
            .where(and(gte(pointTransactions.createdAt, day1Ago), eq(pointTransactions.verified, true), sql `${users.fraudScore} <= 30`, eq(users.banned, false)));
        return {
            totalXP: totalXPResult[0]?.total || 0,
