...
export async function getRateLimitState(userId) {
...
export async function checkVelocity(userId, xpAmount) {
    const oneHourAgo = new Date(Date.now() - 60 * 60 * 1000);
    const recentXP = await db
        .select({ total: sql `COALESCE(SUM(${pointTransactions.amount}), 0)` })
        .from(pointTransactions)
        .where(and(eq(pointTransactions.userId, userId), gte(pointTransactions.createdAt, oneHourAgo)));
...
export async function checkDuplicate(userId, sessionId) {
    const existing = await db.query.pointTransactions.findFirst({
        where: and(eq(pointTransactions.userId, userId), sql `${pointTransactions.metadata}->>'sessionId' = ${sessionId}`)
    });
    if (existing) {
...
// Log violation
async function logViolation(userId, violationType, severity, evidence, fraudScoreImpact) {
...
// Increment fraud score
async function incrementFraudScore(userId, amount) {
...
export async function validateXPSync(userId, xpAmount, sessionId) {
    // Check cooldown
