...
export class PointsEngine {
    async getUserBalance(userId, dbOrTx = db) {
    ...
    async getExpiredUnclaimedPoints(userId, dbOrTx = db) {
        const now = new Date();
        const result = await dbOrTx
            .select({ total: sql `COALESCE(SUM(${pointTransactions.amount}), 0)` })
            .from(pointTransactions)
            .where(and(eq(pointTransactions.userId, userId), eq(pointTransactions.isExpired, false), gte(pointTransactions.amount, 0), sql `${pointTransactions.expiresAt} IS NOT NULL`, sql `${pointTransactions.expiresAt} < NOW()`));
        return Number(result[0]?.total || 0);
    }
    async getExpiringPoints(userId, daysThreshold = 30, dbOrTx = db) {
        const now = new Date();
        ...
        const result = await dbOrTx
            .select({
            total: sql `COALESCE(SUM(${pointTransactions.amount}), 0)`,
            earliest: sql `MIN(${pointTransactions.expiresAt})`
        })
            .from(pointTransactions)
            .where(and(eq(pointTransactions.userId, userId), eq(pointTransactions.isExpired, false), gte(pointTransactions.amount, 0), sql `${pointTransactions.expiresAt} IS NOT NULL`, sql `${pointTransactions.expiresAt} > NOW()`, sql `${pointTransactions.expiresAt} <= NOW() + interval '${sql.raw(daysThreshold.toString())} days'`));
    ...
    async awardPoints(userId, amount, type, sourceId, sourceType, description, dbOrTx) {
        if (amount <= 0)
            throw new Error("Amount must be positive");
        const rule = EARNING_RULES[type.toUpperCase()];
        if (!rule)
            throw new Error("Invalid earning type");
        const executeAward = async (tx) => {
    ...
    async awardSponsoredPoints(userId, amount, challengeId, challengeTitle, dbOrTx) {
        if (amount <= 0)
            throw new Error("Amount must be positive");
        const executeAward = async (tx) => {
            const user = await tx.select().from(users).where(eq(users.id, userId)).limit(1).for("update");
    ...
    async awardMonthlySubscriptionPoints(userId, tier, invoiceId, dbOrTx) {
        const amount = MONTHLY_SUBSCRIPTION_POINTS[tier];
        if (!amount) {
        ...
        const executeAward = async (tx) => {
            ...
            const existing = await tx
                .select()
                .from(pointTransactions)
                .where(and(eq(pointTransactions.userId, userId), eq(pointTransactions.type, "subscription_monthly"), eq(pointTransactions.sourceId, invoiceId)))
                .limit(1);
    ...
    async spendPoints(userId, amount, type, sourceId, sourceType, description, dbOrTx) {
        if (amount <= 0)
            throw new Error("Amount must be positive");
        const executeSpend = async (tx) => {
            const user = await tx.select().from(users).where(eq(users.id, userId)).limit(1).for("update");
            if (!user[0])
                throw new Error("User not found");
            const expiredPoints = await this.getExpiredUnclaimedPoints(userId, tx);
    ...
    async getDailyEarnings(userId, type, dbOrTx = db) {
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const result = await dbOrTx
            .select({ total: sql `COALESCE(SUM(${pointTransactions.amount}), 0)` })
            .from(pointTransactions)
            .where(and(eq(pointTransactions.userId, userId), eq(pointTransactions.type, type), gte(pointTransactions.createdAt, today), gte(pointTransactions.amount, 0)));
        return Number(result[0]?.total || 0);
    }
    async getMonthlyEarnings(userId, dbOrTx = db) {
        ...
        const result = await dbOrTx
            .select({ total: sql `COALESCE(SUM(${pointTransactions.amount}), 0)` })
            .from(pointTransactions)
            .where(and(eq(pointTransactions.userId, userId), gte(pointTransactions.amount, 0), gte(pointTransactions.createdAt, startOfMonth)));
        return Number(result[0]?.total || 0);
    }
    async markExpiredPoints(userId) {
        const now = new Date();
        await db
     ...
     */
    async awardGameplayPoints(userId, minutesPlayed, activityScore, engagementMultiplier = 1.0, verificationScore = 0, desktopSessionId, dbOrTx) {
        if (minutesPlayed <= 0)
            throw new Error("Minutes played must be positive");
        if (activityScore < 0 || activityScore > 100)
            throw new Error("Activity score must be 0-100");
        if (verificationScore < 0 || verificationScore > 100)
            throw new Error("Verification score must be 0-100");
        const executeAward = async (tx) => {
