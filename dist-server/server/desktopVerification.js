 ...
 * }
 */
export async function verifyDesktopMatch(req, res) {
    try {
 ...
 */
async function verifyValorantMatch(matchId, region, puuid) {
    const RIOT_API_KEY = process.env.RIOT_API_KEY;
    if (!RIOT_API_KEY) {
        return { valid: false, error: 'Riot API key not configured' };
    }
    try {
        const response = await fetch(`https://${region}.api.riotgames.com/val/match/v1/matches/${matchId}`, {
            headers: {
        ...
        return {
            valid: true,
            result: player.teamId === data.teams.find((t) => t.won)?.teamId ? 'win' : 'loss',
            details: {
 ...
 */
async function verifyLeagueMatch(matchId, region, puuid) {
    const RIOT_API_KEY = process.env.RIOT_API_KEY;
    if (!RIOT_API_KEY) {
        return { valid: false, error: 'Riot API key not configured' };
    }
    try {
        const response = await fetch(`https://${region}.api.riotgames.com/lol/match/v5/matches/${matchId}`, {
            headers: {
                'X-Riot-Token': RIOT_API_KEY
            }
        });
