...
import { rewardClaims, rewardTypes } from '@shared/schema';
import { eq, and } from 'drizzle-orm';
import { sendEmail } from '../services/email';
/**
 * Automatically fulfill rewards with affiliate links
 */
export async function autoFulfillRewards() {
    const result = {
        fulfilled: 0,
        skipped: 0,
        errors: 0,
    ...
    const pendingClaims = await db
        .select()
        .from(rewardClaims)
        .where(and(eq(rewardClaims.status, 'in_progress'), // Already approved, ready to fulfill
    eq(rewardClaims.fulfilledAt, null) // Not yet fulfilled
    ))
