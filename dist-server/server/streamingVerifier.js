...
import { users, streamingSessions } from "@shared/schema";
...
export class StreamingVerifier {
    ...
    async checkActiveStreams() {
        try {
            ...
            for (const user of linkedUsers) {
                if (!user.twitchUsername || !user.twitchAccessToken)
                    continue;
                try {
    ...
    async verifyUserStream(user) {
        if (!user.twitchUsername || !user.twitchAccessToken)
            return;
        // Check if user is currently streaming
        const streamStatus = await twitchAPI.checkStreamStatus(user.twitchUsername, user.twitchAccessToken);
        ...
        if (!supportedGame) {
            console.log(`[StreamVerifier] ${user.twitchUsername} is streaming ${streamStatus.gameName} - not a supported game`);
            await this.closeActiveSessions(user.id);
            return;
        }
        console.log(`[StreamVerifier] ${user.twitchUsername} is streaming ${streamStatus.gameName} ✓`);
        // Check if there's an active session for this stream
        const activeSession = await db
            .select()
            .from(streamingSessions)
            .where(and(eq(streamingSessions.userId, user.id), eq(streamingSessions.status, 'active'), eq(streamingSessions.twitchStreamId, streamStatus.streamId)))
            .limit(1);
    ...
    async createStreamingSession(userId, streamStatus, supportedGame) {
        try {
            ...
            console.log(`[StreamVerifier] Created new streaming session for user ${userId} - ${supportedGame.name}`);
        }
        catch (error) {
            console.error('[StreamVerifier] Error creating streaming session:', error);
        }
    }
    async updateStreamingSession(session, streamStatus, supportedGame) {
        try {
            const now = new Date();
            ...
            if (newPoints > 0) {
                ...
                console.log(`[StreamVerifier] Updated session for user ${session.userId} - awarded ${newPoints} points (${totalMinutesStreamed} min total, ${fullHoursToAward} new hours)`);
            }
            else {
    ...
    async closeActiveSessions(userId) {
        try {
            const activeSessions = await db
                .select()
                .from(streamingSessions)
                .where(and(eq(streamingSessions.userId, userId), eq(streamingSessions.status, 'active')));
    ...
    async awardStreamingPoints(userId, points, gameName) {
        try {
            // Use pointsEngine to award points with proper validation and monthly caps
            await pointsEngine.awardPoints(userId, points, "STREAMING", undefined, "streaming", `Streamed ${gameName} on Twitch`);
