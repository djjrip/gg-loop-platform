 ...
 */
export async function analyzeChurnRisk(userId) {
 ...
 */
export async function getSmartRecommendations(userId) {
    const user = await db.query.users.findFirst({
        where: eq(users.id, userId)
    });
    if (!user)
    ...
    const affordableRewards = await db.query.rewards.findMany({
        where: and(sql `${rewards.pointsCost} <= ${pointsBalance}`, eq(rewards.isActive, true)),
    ...
    const redeemedCategories = new Set(userRedemptions.map(r => r.rewardCategory).filter(Boolean));
...
export async function checkEngagementTriggers(userId) {
    const user = await db.query.users.findFirst({
        where: eq(users.id, userId)
    });
    if (!user)
