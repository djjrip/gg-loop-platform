 ...
 */
export async function linkSteamAccount(userId, steamId) {
 ...
 */
async function verifySteamId(steamId) {
    try {
        const response = await fetch(`${STEAM_API_BASE}/ISteamUser/GetPlayerSummaries/v2/?key=${STEAM_API_KEY}&steamids=${steamId}`);
...
export async function getRecentlyPlayedGames(steamId) {
    const response = await fetch(`${STEAM_API_BASE}/IPlayerService/GetRecentlyPlayedGames/v1/?key=${STEAM_API_KEY}&steamid=${steamId}&count=10`);
    const data = await response.json();
 ...
 */
export async function calculateSteamPoints(steamId, lastCheckDate) {
...
export async function getCS2Stats(steamId) {
    try {
        const response = await fetch(`${STEAM_API_BASE}/ISteamUserStats/GetUserStatsForGame/v2/?appid=730&key=${STEAM_API_KEY}&steamid=${steamId}`);
        const data = await response.json();
        const stats = data.playerstats.stats;
        return {
            kills: stats.find((s) => s.name === 'total_kills')?.value || 0,
            deaths: stats.find((s) => s.name === 'total_deaths')?.value || 0,
            wins: stats.find((s) => s.name === 'total_wins')?.value || 0,
            mvps: stats.find((s) => s.name === 'total_mvps')?.value || 0,
        };
    }
    catch (error) {
