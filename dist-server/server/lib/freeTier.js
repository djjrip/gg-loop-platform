 ...
 */
export async function awardGgCoins(userId, amount, reason, sourceType, sourceId) {
...
export async function updateLoginStreak(userId) {
    ...
    if (newStreak % 7 === 0 && newStreak > (user.loginStreak || 0)) {
        ...
        await awardGgCoins(userId, coinsAwarded, `${newStreak}-day login streak milestone!`, 'streak', String(newStreak));
    }
 ...
 */
export async function unlockBadgeByCondition(userId, condition) {
    const [badge] = await db
        .select()
        .from(virtualBadges)
        .where(and(eq(virtualBadges.unlockCondition, condition), eq(virtualBadges.isActive, true)))
        .limit(1);
    if (!badge)
        return undefined;
    ...
    if (badge.ggCoinsReward > 0) {
        await awardGgCoins(userId, badge.ggCoinsReward, `Badge unlocked: ${badge.name}`, 'badge_unlock', badge.id);
    }
    return badge.name;
}
/**
 ...
 */
export async function redeemBasicTrial(userId) {
 ...
 */
export async function getFreeTierStatus(userId) {
    const [user] = await db.select().from(users).where(eq(users.id, userId)).limit(1);
    if (!user)
        throw new Error("User not found");
