...
export async function getRevenueMetrics() {
    ...
    const [proCount] = await db.select({ count: sql `count(*)` })
        .from(users)
        .where(and(eq(users.subscriptionStatus, 'active'), eq(users.subscriptionTier, 'pro')));
    const [eliteCount] = await db.select({ count: sql `count(*)` })
        .from(users)
        .where(and(eq(users.subscriptionStatus, 'active'), eq(users.subscriptionTier, 'elite')));
    const [trialingCount] = await db.select({ count: sql `count(*)` })
        .from(users)
        .where(eq(users.subscriptionStatus, 'trialing'));
    const [freeCount] = await db.select({ count: sql `count(*)` })
        .from(users)
    ...
    const [lastMonthSubs] = await db.select({ count: sql `count(*)` })
        .from(users)
        .where(and(eq(users.subscriptionStatus, 'active'), gte(users.createdAt, lastMonth)));
...
export async function getConversionMetrics() {
    ...
    const [conversions] = await db.select({ count: sql `count(*)` })
        .from(users)
        .where(and(eq(users.subscriptionStatus, 'active'), sql `trial_ends_at IS NOT NULL` // Had a trial
    ));
    // Total trial users ever
    const [totalTrials] = await db.select({ count: sql `count(*)` })
        .from(users)
    ...
    const [freeConversions] = await db.select({ count: sql `count(*)` })
        .from(users)
        .where(and(eq(users.subscriptionStatus, 'active'), sql `trial_ends_at IS NULL`));
    const [totalFree] = await db.select({ count: sql `count(*)` })
        .from(users)
        .where(eq(users.subscriptionStatus, 'inactive'));
...
export async function getUserMetrics() {
    ...
    const [signupsToday] = await db.select({ count: sql `count(*)` })
        .from(users)
        .where(gte(users.createdAt, today));
    const weekAgo = new Date();
    weekAgo.setDate(weekAgo.getDate() - 7);
    const [signupsThisWeek] = await db.select({ count: sql `count(*)` })
        .from(users)
        .where(gte(users.createdAt, weekAgo));
    const monthAgo = new Date();
    monthAgo.setMonth(monthAgo.getMonth() - 1);
    const [signupsThisMonth] = await db.select({ count: sql `count(*)` })
        .from(users)
        .where(gte(users.createdAt, monthAgo));
    const twoWeeksAgo = new Date();
    twoWeeksAgo.setDate(twoWeeksAgo.getDate() - 14);
    const [lastWeekSignups] = await db.select({ count: sql `count(*)` })
        .from(users)
        .where(and(gte(users.createdAt, twoWeeksAgo), sql `created_at < ${weekAgo}`));
