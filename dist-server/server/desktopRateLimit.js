 ...
 */
export function checkRateLimit(userId, matchId) {
    const now = Date.now();
    const userHistory = verificationHistory.get(userId) || [];
    // Clean old entries (older than 24 hours)
    const last24Hours = userHistory.filter(attempt => now - attempt.timestamp < 24 * 60 * 60 * 1000);
    // Check daily limit
    ...
    const lastHour = userHistory.filter(attempt => now - attempt.timestamp < 60 * 60 * 1000);
 ...
 */
export function recordVerification(userId, matchId) {
    const userHistory = verificationHistory.get(userId) || [];
    userHistory.push({
        userId,
        timestamp: Date.now(),
    ...
    const last24Hours = userHistory.filter(attempt => Date.now() - attempt.timestamp < 24 * 60 * 60 * 1000);
    verificationHistory.set(userId, last24Hours);
}
/**
 * Get user's verification stats
 */
export function getVerificationStats(userId) {
    const now = Date.now();
    const userHistory = verificationHistory.get(userId) || [];
    const last24Hours = userHistory.filter(attempt => now - attempt.timestamp < 24 * 60 * 60 * 1000);
    const lastHour = userHistory.filter(attempt => now - attempt.timestamp < 60 * 60 * 1000);
...
export function getSuspiciousUsers() {
    ...
    for (const [userId, history] of verificationHistory.entries()) {
        const last24Hours = history.filter(attempt => now - attempt.timestamp < 24 * 60 * 60 * 1000);
        const lastHour = history.filter(attempt => now - attempt.timestamp < 60 * 60 * 1000);
