...
// ============================================
export async function checkAndGrantFoundingMember(userId) {
    // Count total users
...
// ============================================
export async function getFoundingMemberStatus(userId) {
    const user = await db
    ...
    if (!user[0]?.metadata)
        return null;
    const metadata = JSON.parse(user[0].metadata);
    return metadata.foundingMember || null;
}
// ============================================
// FOUNDING MEMBER WELCOME EMAIL
// ============================================
export function generateFoundingMemberEmail(badgeNumber, username) {
    return `
Hey ${username},
...
// ============================================
export async function getFoundingSpotsRemaining() {
    const totalUsers = await db
        .select({ count: sql `count(*)` })
        .from(users);
    const userCount = Number(totalUsers[0]?.count || 0);
