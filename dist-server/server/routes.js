...
import { users, userGames, riotAccounts, sponsors, insertSponsorSchema, challenges, challengeCompletions, insertChallengeSchema, insertUserGameSchema, insertRewardSchema, insertUserRewardSchema, userRewards, rewards, subscriptions, rewardClaims, rewardTypes, matchWinWebhookSchema, achievementWebhookSchema, tournamentWebhookSchema, processedRiotMatches, referrals, affiliateApplications, charities, charityCampaigns, games, leaderboardEntries, verificationProofs, fraudDetectionLogs, verificationQueue, gameRequests, insertGameRequestSchema, pointTransactions } from "@shared/schema";
...
export async function registerRoutes(app) {
    ...
    app.get("/api/rewards/catalog", async (req, res, next) => {
        req.url = "/catalog";
        rewardRedemptionRouter(req, res, next);
    });
    ...
    app.post("/api/admin/mission-tweets", async (req, res) => {
        ...
        console.log(`[${runId}] MISSION_TWEETS_TRIGGERED`);
        ...
        if (!consumerKey || !consumerSecret || !accessToken || !accessSecret) {
            ...
            return res.status(500).json({
                ...
                tweeted: false,
                hint: "Add TWITTER_CONSUMER_KEY, TWITTER_CONSUMER_SECRET, TWITTER_ACCESS_TOKEN, TWITTER_ACCESS_SECRET to Railway Variables."
            });
        }
        try {
            ...
            console.log(`[${runId}] MISSION_TWITTER_ME`, { username, userId });
            ...
            if (isDryRun) {
                ...
                return res.json({
                    ok: true,
                    tweeted: false,
                    message: "Keys Valid. Dry Run Complete.",
                    username,
                    tweetCount: outboundConfig.missionTweets.length
                });
            }
            // 3. Post Tweets
            ...
            for (const [i, tweetText] of tweets.entries()) {
                console.log(`[${runId}] Posting tweet ${i + 1}/${tweets.length}...`);
                const response = await client.v2.tweet(tweetText);
                console.log(`[${runId}] MISSION_TWEET_POSTED`, { tweetId: response.data.id });
                results.push({
                    id: response.data.id,
    ...
    app.get('/healthcheck', (req, res) => {
        ...
        res.send("GGLOOP_DEPLOY_VERSION=2025-12-19T11:45Z");
    });
    // END PATCH
    ...
    app.get('/api/test/admin-login', async (req, res) => {
        ...
        try {
            ...
            res.status(500).send('Failed to create admin session: ' + error.message);
        }
    });
    ...
    app.get('/api/admin/dashboard-stats', adminMiddleware, async (req, res) => {
        try {
            ...
            // Get founder count
            const founderResult = await db.select({ count: sql `COUNT(*)` }).from(users).where(eq(users.isFounder, true));
            const founderCount = Number(founderResult[0]?.count || 0);
            // Get users active today (logged in today)
            const today = new Date();
            today.setHours(0, 0, 0, 0);
            const activeResult = await db.select({ count: sql `COUNT(*)` }).from(users).where(gte(users.lastLoginAt, today));
            const activeToday = Number(activeResult[0]?.count || 0);
            // Get total points issued
            const pointsResult = await db.select({ total: sql `COALESCE(SUM(${users.totalPoints}), 0)` }).from(users);
            const totalPoints = Number(pointsResult[0]?.total || 0);
            // Get total rewards claimed
            const rewardsResult = await db.select({ count: sql `COUNT(*)` }).from(userRewards);
    ...
    app.post('/api/admin/config/env', adminMiddleware, async (req, res) => {
        try {
    ...
    app.get('/api/admin/empire-stats', adminMiddleware, async (req, res) => {
        try {
            ...
            monthlyRevenue = activeSubs.reduce((sum, sub) => sum + (tierPricing[sub.tier] || 0), 0);
            // USER COUNT
            ...
            const vibeSessions = await db.select({ count: sql `COUNT(*)` })
    ...
    app.get('/api/admin/revenue-metrics', adminMiddleware, async (req, res) => {
        try {
            // Get total users
            const totalUsersResult = await db.select({ count: sql `COUNT(*)` }).from(users);
            const totalUsers = Number(totalUsersResult[0]?.count || 0);
    ...
    app.get('/api/admin/revenue/snapshot', adminMiddleware, async (req, res) => {
        try {
    ...
    app.get('/api/admin/revenue/daily', adminMiddleware, async (req, res) => {
        try {
            const { revenueTracker } = await import("./monitoring/revenueTracker");
    ...
    app.get('/api/admin/twitter/mentions', adminMiddleware, async (req, res) => {
        try {
    ...
    app.get('/api/admin/twitter/engagement', adminMiddleware, async (req, res) => {
        try {
            const { twitterMonitor } = await import("./services/twitterMonitor");
    ...
    app.get('/admin/command-center', adminMiddleware, async (req, res) => {
        try {
            const fs = await import('fs/promises');
            const path = await import('path');
    ...
    app.get('/api/sponsors/eligibility', requireAuth, async (req, res) => {
        try {
    ...
    app.post('/api/verification/submit-proof', requireAuth, async (req, res) => {
        try {
            const userId = req.dbUser.id;
            const { sourceType, sourceId, fileUrl, fileType, fileSizeBytes, fileMetadata } = req.body;
            const proof = await verificationService.handleProofUpload(userId, sourceType, sourceId, fileUrl, fileType, fileSizeBytes, fileMetadata);
    ...
    app.post("/api/verification/upload-url", requireAuth, async (req, res) => {
        try {
            const userId = req.dbUser.id;
            ...
            const result = await generatePresignedUploadUrl(userId, extension, contentType || 'application/octet-stream');
    ...
    app.get('/api/verification/queue', adminMiddleware, async (req, res) => {
        try {
            const { status, priority, limit = 50 } = req.query;
            const queueItems = await db
                .select()
                .from(verificationQueue)
                .where(and(status ? eq(verificationQueue.status, status) : undefined, priority ? eq(verificationQueue.priority, Number(priority)) : undefined))
    ...
    app.post('/api/verification/review/:id', adminMiddleware, async (req, res) => {
        try {
            const { id } = req.params;
            const { action, notes } = req.body; // action: 'approve' | 'reject' | 'flag'
            const adminId = req.dbUser.id;
            const result = await verificationService.processAdminReview(Number(id), adminId, action, notes);
            res.json({ success: true, result });
        }
        catch (error) {
    ...
    app.get('/api/verification/stats', adminMiddleware, async (req, res) => {
        try {
            ...
            const [highRiskCount] = await db
                ...
                .where(and(gte(fraudDetectionLogs.riskScore, 70), eq(fraudDetectionLogs.status, 'pending')));
            res.json({
    ...
    app.post('/api/verification/bulk-action', adminMiddleware, async (req, res) => {
        try {
            ...
            for (const itemId of itemIds) {
                try {
                    const result = await verificationService.processAdminReview(itemId, adminId, action, `Bulk ${action}`);
    ...
    app.get('/api/verification/fraud-alerts', adminMiddleware, async (req, res) => {
        try {
            const { severity, limit = 50 } = req.query;
            const alerts = await fraudDetectionService.getActiveFraudAlerts(severity, Number(limit));
    ...
    app.post('/api/desktop/session/start', requireAuth, async (req, res) => {
        try {
            const userId = req.dbUser.id;
    ...
    app.post('/api/desktop/session/end', requireAuth, async (req, res) => {
        try {
            const userId = req.dbUser.id;
    ...
    app.post('/api/desktop/verification/payload', requireAuth, async (req, res) => {
        try {
            const userId = req.dbUser.id;
    ...
    // Heartbeat endpoint
    app.post('/api/desktop/heartbeat', requireAuth, async (req, res) => {
        try {
    ...
    app.post('/api/rewards/claim', requireAuth, async (req, res) => {
        try {
            ...
            const hasDesktopSession = await db
                .select()
                .from(verificationProofs)
                .where(and(eq(verificationProofs.userId, userId), eq(verificationProofs.desktopVerified, true)))
                .limit(1);
            ...
            const [claim] = await db
                ...
                .returning();
            // Deduct points
            await pointsEngine.spendPoints(userId, reward.costInPoints, 'reward_claim', claim.id.toString(), 'reward', `Claimed: ${reward.name}`);
    ...
    // Get claim status
    app.get('/api/rewards/claim/status/:id', requireAuth, async (req, res) => {
        try {
            const userId = req.dbUser.id;
            ...
            const [claim] = await db
                .select()
                .from(userRewards)
                .where(and(eq(userRewards.id, claimId), eq(userRewards.userId, userId)));
            if (!claim) {
    ...
    app.post('/api/admin/rewards/approve/:claimId', adminMiddleware, async (req, res) => {
        try {
    ...
    app.post('/api/admin/rewards/reject/:claimId', adminMiddleware, async (req, res) => {
        try {
            const claimId = parseInt(req.params.claimId);
            const adminId = req.dbUser.id;
            const [claim] = await db
            ...
            await pointsEngine.awardPoints(claim.userId, claim.pointsCost || 0, 'REWARD_REFUND', claimId.toString(), 'reward_claim', `Refund for rejected claim #${claimId}`);
            // Update claim status
            await db
                .update(userRewards)
                .set({
    ...
    // Unlock brand tier
    app.post('/api/brands/unlock', requireAuth, async (req, res) => {
        try {
            const userId = req.dbUser.id;
    ...
    app.get('/api/admin/brands/pending', adminMiddleware, async (req, res) => {
        try {
    ...
    app.post('/api/admin/brands/approve/:id', adminMiddleware, async (req, res) => {
        try {
    ...
    app.post('/api/admin/brands/reject/:id', adminMiddleware, async (req, res) => {
        try {
            const brandId = req.params.id;
            const adminId = req.dbUser.id;
    ...
    app.get('/api/passport/stats', requireAuth, async (req, res) => {
        try {
            const userId = req.dbUser.id;
            // Get user points
            ...
            const rewardsClaimedResult = await db
                .select({ count: sql `COUNT(*)` })
                .from(userRewards)
                .where(and(eq(userRewards.userId, userId), eq(userRewards.status, 'fulfilled')));
    ...
    app.get('/api/passport/history', requireAuth, async (req, res) => {
        try {
            const userId = req.dbUser.id;
    ...
    app.post('/api/passport/feedback', requireAuth, async (req, res) => {
        try {
            const userId = req.dbUser.id;
    ...
    app.get('/api/public/platform-stats', async (req, res) => {
        try {
            ...
            const activeSubscriptionsResult = await db
                ...
                .where(eq(subscriptions.status, 'active'));
            const activeSubscriptions = Number(activeSubscriptionsResult[0]?.count || 0);
            const totalRewardsResult = await db.select({ count: sql `COUNT(*)` }).from(rewards);
            const totalRewards = Number(totalRewardsResult[0]?.count || 0);
            const rewardsRedeemedResult = await db.select({ count: sql `COUNT(*)` }).from(userRewards);
            const rewardsRedeemed = Number(rewardsRedeemedResult[0]?.count || 0);
            const totalReferralsResult = await db.select({ count: sql `COUNT(*)` }).from(referrals);
            const totalReferrals = Number(totalReferralsResult[0]?.count || 0);
            const gamesResult = await db.select({ count: sql `COUNT(*)` }).from(games);
    ...
    app.get('/api/auth/login-notification', async (req, res) => {
        try {
    ...
    app.get('/api/twitch/auth', isAuthenticated, (req, res) => {
        try {
    ...
    app.get('/api/twitch/callback', isAuthenticated, async (req, res) => {
        try {
            ...
            const baseUrl = process.env.BASE_URL || 'http://localhost:5000';
            const redirectUri = `${baseUrl}/api/auth/twitch/callback`;
            const tokens = await twitchAPI.exchangeCodeForTokens(code, redirectUri);
    ...
    app.post('/api/twitch/disconnect', isAuthenticated, getUserMiddleware, async (req, res) => {
        try {
    ...
    app.post('/api/riot/:gameId/request-code', isAuthenticated, getUserMiddleware, async (req, res) => {
        try {
    ...
    app.post('/api/riot/:gameId/verify', isAuthenticated, getUserMiddleware, async (req, res) => {
        try {
            const { gameId } = req.params;
    ...
    app.post('/api/riot/:gameId/link-provisional', isAuthenticated, getUserMiddleware, async (req, res) => {
        try {
            const { gameId } = req.params;
            const { riotId, region = 'na' } = req.body;
    ...
    app.post('/api/riot/:gameId/disconnect', isAuthenticated, getUserMiddleware, async (req, res) => {
        try {
            const { gameId } = req.params;
            const userId = req.dbUser.id;
    ...
    app.get('/api/riot/:gameId/status', isAuthenticated, getUserMiddleware, async (req, res) => {
        try {
            const { gameId } = req.params;
    ...
    app.get('/api/user/games', getUserMiddleware, async (req, res) => {
        try {
            const userId = req.dbUser.id;
            const userGames = await storage.getUserGames(userId);
            res.json(userGames);
    ...
    app.get('/api/user/expiring-points', getUserMiddleware, async (req, res) => {
        try {
            const userId = req.dbUser.id;
    ...
    app.post('/api/user/username', getUserMiddleware, async (req, res) => {
        try {
            const userId = req.dbUser.id;
    ...
    app.post('/api/user/shipping-address', getUserMiddleware, async (req, res) => {
        try {
            const userId = req.dbUser.id;
    ...
    app.post('/api/user/games', getUserMiddleware, async (req, res) => {
        try {
            const userId = req.dbUser.id;
    ...
    app.post('/api/riot/link-account-DISABLED', getUserMiddleware, async (req, res) => {
    ...
    app.post('/api/riot/link-account-secure', getUserMiddleware, async (req, res) => {
        try {
            const userId = req.dbUser.id;
    ...
    app.post('/api/riot/link/league', getUserMiddleware, async (req, res) => {
        try {
            const userId = req.dbUser.id;
            ...
            const [existingAccount] = await db.select().from(riotAccounts).where(and(eq(riotAccounts.userId, userId), eq(riotAccounts.game, 'league')));
            ...
            if (existingAccount && existingAccount.puuid !== riotAccount.puuid) {
                ...
                await db.delete(processedRiotMatches).where(eq(processedRiotMatches.riotAccountId, existingAccount.id));
                ...
                if (leagueGame) {
                    await db.delete(leaderboardEntries).where(and(eq(leaderboardEntries.userId, userId), eq(leaderboardEntries.gameId, leagueGame.id)));
                }
            }
    ...
    app.post('/api/riot/link/valorant', getUserMiddleware, async (req, res) => {
        try {
            const userId = req.dbUser.id;
            ...
            const [existingAccount] = await db.select().from(riotAccounts).where(and(eq(riotAccounts.userId, userId), eq(riotAccounts.game, 'valorant')));
            // If switching to a different player (different PUUID), delete old match history and leaderboard entries
            // This prevents showing stats from a different player's account
            if (existingAccount && existingAccount.puuid !== riotAccount.puuid) {
                ...
                if (valorantGame) {
                    await db.delete(leaderboardEntries).where(and(eq(leaderboardEntries.userId, userId), eq(leaderboardEntries.gameId, valorantGame.id)));
                }
            }
            // Save to database (update if exists, insert if new)
            await db.insert(riotAccounts).values({
    ...
    app.get('/api/riot/account/:game', getUserMiddleware, async (req, res) => {
        try {
            const userId = req.dbUser.id;
            const { game } = req.params;
            const [riotAccount] = await db.select().from(riotAccounts).where(and(eq(riotAccounts.userId, userId), eq(riotAccounts.game, game)));
            if (!riotAccount) {
            ...
            const matches = await db.select({
                ...
                .limit(1);
            const totalMatches = await db.select({ count: sql `count(*)::int` })
                .from(processedRiotMatches)
                .where(eq(processedRiotMatches.riotAccountId, riotAccount.id));
            const wins = await db.select({ count: sql `count(*)::int` })
                .from(processedRiotMatches)
                .where(and(eq(processedRiotMatches.riotAccountId, riotAccount.id), eq(processedRiotMatches.isWin, true)));
    ...
    app.get('/api/riot/matches', getUserMiddleware, async (req, res) => {
        try {
            const userId = req.dbUser.id;
            const limit = req.query.limit ? parseInt(req.query.limit) : 20;
            const userRiotAccounts = await db.select().from(riotAccounts).where(eq(riotAccounts.userId, userId));
            ...
            const matches = await db.select({
                ...
                .where(inArray(processedRiotMatches.riotAccountId, accountIds))
    ...
    app.get('/api/activity/sync-status', async (req, res) => {
        try {
            const lastProcessed = await db.select({
                ...
                .orderBy(desc(processedRiotMatches.processedAt))
                .limit(1);
            const totalLinkedAccounts = await db.select({ count: sql `count(*)::int` })
                .from(riotAccounts);
    ...
    app.get('/api/user/achievements', getUserMiddleware, async (req, res) => {
        try {
            const userId = req.dbUser.id;
            const limit = req.query.limit ? parseInt(req.query.limit) : 10;
    ...
    app.get('/api/user/rewards', getUserMiddleware, async (req, res) => {
        try {
            const userId = req.dbUser.id;
    ...
    app.get('/api/admin/pending-rewards', adminMiddleware, async (req, res) => {
        try {
    ...
    app.get('/api/admin/daily-metrics', adminMiddleware, async (req, res) => {
        try {
    ...
    app.get('/api/public/daily-metrics', requireAuth, async (req, res) => {
        try {
            const metrics = await storage.getDailyMetrics();
    ...
    app.get('/api/admin/checklist/:date', adminMiddleware, async (req, res) => {
        try {
    ...
    app.post('/api/admin/checklist/toggle', adminMiddleware, async (req, res) => {
        try {
    ...
    app.post('/api/admin/rewards/fulfill', adminMiddleware, async (req, res) => {
        try {
            const { userRewardId, giftCardCode } = z.object({
                userRewardId: z.string(),
                giftCardCode: z.string().optional()
            }).parse(req.body);
            const userReward = await storage.updateUserRewardStatus(userRewardId, 'fulfilled', giftCardCode ? { giftCardCode, fulfilledAt: new Date() } : { fulfilledAt: new Date() });
            res.json(userReward);
        }
        catch (error) {
    ...
    app.post('/api/admin/rewards/tracking', adminMiddleware, async (req, res) => {
        try {
    ...
    app.get('/api/admin/redemptions', adminMiddleware, async (req, res) => {
        try {
    ...
    app.post('/api/admin/fulfill-redemption', adminMiddleware, async (req, res) => {
        try {
            ...
            const fulfillmentData = { giftCardCode };
            const updateData = {
                status: 'fulfilled',
                fulfilledAt: new Date(),
    ...
    app.patch('/api/admin/rewards/:id', adminMiddleware, async (req, res) => {
        try {
    ...
    app.post('/api/user/rewards/redeem', getUserMiddleware, async (req, res) => {
        try {
            const userId = req.dbUser.id;
            ...
            if (reward.fulfillmentType === 'physical') {
                ...
                if (!user.shippingCity?.trim())
                    missingFields.push("City");
                if (!user.shippingState?.trim())
                    missingFields.push("State/Province");
                if (!user.shippingZip?.trim())
...
ACTION NEEDED: ${reward.fulfillmentType === 'physical'
    ...
    app.get('/api/points/balance', getUserMiddleware, async (req, res) => {
        try {
            const userId = req.dbUser.id;
    ...
    app.get('/api/points/transactions', getUserMiddleware, async (req, res) => {
        try {
            const userId = req.dbUser.id;
    ...
    app.post('/api/user/rewards/redeem', getUserMiddleware, async (req, res) => {
        try {
            ...
            const result = await db.transaction(async (tx) => {
                ...
                await pointsEngine.spendPoints(userId, reward.pointsCost, "reward_redemption", reward.id, "reward", `Redeemed: ${reward.title}`, tx);
    ...
    app.get('/api/match-submissions', getUserMiddleware, async (req, res) => {
        try {
            const userId = req.dbUser.id;
    ...
    app.post('/api/match-submissions', getUserMiddleware, async (req, res) => {
        try {
            const userId = req.dbUser.id;
            ...
            const matchIds = await riotApi.getRecentMatchIds(riotAccount.riotPuuid, riotAccount.riotRegion || 'na', 20);
            ...
            for (const matchId of matchIds) {
                ...
                if (participant && participant.win) {
                    ...
                    await db.transaction(async (tx) => {
                        // Award points
                        await pointsEngine.awardPoints(userId, pointsAwarded, 'MATCH_WIN', submission.id, 'match_submission', `Verified ${matchType} win - ${participant.championName}`, tx);
                        // Auto-track challenge progress (atomic)
                        const now = new Date();
                        const activeChallenges = await tx
                            .select()
                            .from(challenges)
                            .where(and(eq(challenges.isActive, true), eq(challenges.requirementType, 'match_wins'), sql `${challenges.startDate} <= ${now}`, sql `${challenges.endDate} >= ${now}`));
    ...
    app.get('/api/challenges', async (req, res) => {
        try {
            ...
            const activeChallenges = await db
                .select()
                .from(challenges)
                .where(and(eq(challenges.isActive, true), sql `${challenges.startDate} <= ${now}`, sql `${challenges.endDate} >= ${now}`));
            ...
            const enriched = await Promise.all(activeChallenges.map(async (challenge) => {
                ...
                const [completion] = await db
                    ...
                    .where(and(eq(challengeCompletions.userId, userId), eq(challengeCompletions.challengeId, challenge.id)))
                    .limit(1);
                return {
                    ...challenge,
    ...
    app.post('/api/challenges/:id/claim', getUserMiddleware, async (req, res) => {
        try {
            const userId = req.dbUser.id;
            ...
            const result = await db.transaction(async (tx) => {
                ...
                const transaction = await pointsEngine.awardSponsoredPoints(userId, challenge.bonusPoints, challenge.id, challenge.title, tx);
    ...
    app.post('/api/admin/challenges', adminMiddleware, async (req, res) => {
        try {
            ...
            if (challengeData.sponsorId) {
                ...
                const activeChallenges = await db.select()
                    .from(challenges)
                    .where(and(eq(challenges.sponsorId, challengeData.sponsorId), eq(challenges.isActive, true)));
                // Reserved budget = sum of (totalBudget - pointsDistributed) for active challenges
                const reservedBudget = activeChallenges.reduce((sum, c) => sum + (c.totalBudget - c.pointsDistributed), 0);
    ...
    app.post('/api/paypal/create-subscription', getUserMiddleware, async (req, res) => {
    ...
    app.post('/api/paypal/manual-sync', getUserMiddleware, async (req, res) => {
    ...
    app.post('/api/paypal/subscription-approved', getUserMiddleware, async (req, res) => {
        const { subscriptionId } = req.body;
        const userId = req.dbUser.id;
        if (!subscriptionId) {
    ...
    app.post('/api/subscription/cancel', getUserMiddleware, async (req, res) => {
        const userId = req.dbUser.id;
        try {
            const subscription = await storage.getSubscription(userId);
            if (!subscription) {
            ...
            if (subscription.paypalSubscriptionId) {
                await cancelPayPalSubscription(subscription.paypalSubscriptionId, "User requested cancellation");
            }
    ...
    app.post('/api/webhooks/gaming/match-win', webhookAuth, async (req, res) => {
        try {
            ...
            try {
                const transaction = await pointsEngine.awardPoints(userId, pointsToAward, "MATCH_WIN", event.id, "gaming_event", `Match win - ${partner.name}`);
    ...
    app.post('/api/webhooks/gaming/achievement', webhookAuth, async (req, res) => {
        try {
            ...
            try {
                const transaction = await pointsEngine.awardPoints(userId, pointsToAward, "ACHIEVEMENT", event.id, "gaming_event", achievementData.title || `Achievement - ${partner.name}`);
                await storage.updateGamingEvent(event.id, {
                    status: "processed",
                    pointsAwarded: pointsToAward,
                    transactionId: transaction.id,
    ...
    app.post('/api/webhooks/gaming/tournament', webhookAuth, async (req, res) => {
        try {
            ...
            if (tournamentData.placement === 1)
                pointsToAward = 100;
            else if (tournamentData.placement === 2)
                pointsToAward = 75;
            else if (tournamentData.placement === 3)
                pointsToAward = 50;
            const event = await storage.logGamingEvent({
                partnerId: partner.id,
                userId,
            ...
            try {
                const transaction = await pointsEngine.awardPoints(userId, pointsToAward, "TOURNAMENT_PLACEMENT", event.id, "gaming_event", `Tournament placement #${tournamentData.placement} - ${partner.name}`);
                await storage.updateGamingEvent(event.id, {
                    status: "processed",
                    pointsAwarded: pointsToAward,
                    transactionId: transaction.id,
    ...
    app.post('/api/referrals/start-trial', isAuthenticated, getUserMiddleware, async (req, res) => {
        try {
            const userId = req.dbUser.id;
    ...
    app.get('/api/referrals/my-stats', isAuthenticated, getUserMiddleware, async (req, res) => {
        try {
            const userId = req.dbUser.id;
    ...
    app.post('/api/referrals/check-squad-milestones', isAuthenticated, getUserMiddleware, async (req, res) => {
        try {
            const userId = req.dbUser.id;
            ...
            for (const { milestone, earned } of milestoneResults) {
                if (earned) {
                    await pointsEngine.awardPoints(userId, milestone.bonusPoints, 'squad_milestone', userId, `Squad Milestone: ${milestone.name}`);
    ...
    app.post('/api/referrals/complete/:userId', async (req, res) => {
        try {
            ...
            const transaction = await pointsEngine.awardPoints(user.referredBy, pointsForThisReferral, "REFERRAL", referral.id, "referral", `Referral bonus - ${user.username || 'user'} subscribed`);
    ...
    app.post('/api/admin/verify-streams', adminMiddleware, async (req, res) => {
        try {
    ...
    app.post('/api/admin/verify-user-stream/:userId', adminMiddleware, async (req, res) => {
        try {
            const { streamingVerifier } = await import("./streamingVerifier");
    ...
    // Free Tier Routes
    app.get('/api/free-tier/status', getUserMiddleware, async (req, res) => {
        try {
    ...
    app.post('/api/free-tier/redeem-trial', getUserMiddleware, async (req, res) => {
        try {
    ...
    app.get('/api/admin/sponsors', adminMiddleware, async (req, res) => {
        try {
    ...
    app.post('/api/admin/sponsors', adminMiddleware, async (req, res) => {
        try {
    ...
    app.patch('/api/admin/sponsors/:id', adminMiddleware, async (req, res) => {
        try {
            const { id } = req.params;
    ...
    app.delete('/api/admin/sponsors/:id', adminMiddleware, async (req, res) => {
        try {
            const { id } = req.params;
    ...
    app.post('/api/admin/sponsors/:id/add-budget', adminMiddleware, async (req, res) => {
        try {
            const { id } = req.params;
    ...
    app.post('/api/admin/sponsors/migrate', adminMiddleware, async (req, res) => {
        try {
    ...
    app.post('/api/admin/sponsors/sync-budgets', adminMiddleware, async (req, res) => {
        try {
    ...
    // Sponsor Analytics
    app.get('/api/admin/sponsors/:id/analytics', adminMiddleware, async (req, res) => {
        try {
            const { id } = req.params;
            // Get sponsor
            const [sponsor] = await db.select()
            ...
            const completedUsers = new Set(completions
                .filter(c => {
            ...
            const claimedUsers = new Set(completions
                .filter(c => c.claimed)
                .map(c => c.userId));
            ...
            const challengeAnalytics = sponsorChallenges.map((challenge) => {
                ...
                const completed = new Set(challengeCompletionData
                    .filter(c => c.progress >= challenge.requirementCount)
                    .map(c => c.userId)).size;
                // Unique users who claimed
                const claimed = new Set(challengeCompletionData
                    .filter(c => c.claimed)
                    .map(c => c.userId)).size;
    ...
    app.post('/api/admin/points/adjust', adminMiddleware, async (req, res) => {
        try {
            ...
            const result = await adjustUserPoints(adminUser.id, adminUser.email || 'unknown', targetUserId, amount, reason, ipAddress);
            res.json({
                ...result,
                message: amount > 0
    ...
    app.get('/api/admin/system-health', adminMiddleware, async (req, res) => {
        try {
    ...
    app.post('/api/admin/check-spending-limit', adminMiddleware, async (req, res) => {
        try {
            const schema = z.object({
    ...
    app.post('/api/tiktok/track-copy', getUserMiddleware, async (req, res) => {
        try {
            ...
            const existingTransaction = await db.select()
                ...
                .where(and(eq(require('@shared/schema').pointTransactions.userId, userId), eq(require('@shared/schema').pointTransactions.sourceType, 'tiktok_template'), eq(require('@shared/schema').pointTransactions.sourceId, templateId)))
                .limit(1);
            ...
            await pointsEngine.awardPoints(userId, POINTS_PER_TEMPLATE, "CONTENT_CREATION", templateId, "tiktok_template", `Created TikTok content using template #${templateId}`);
            res.json({
    ...
    app.get('/api/affiliate/stats', getUserMiddleware, async (req, res) => {
        try {
            const userId = req.dbUser.id;
    ...
    app.post('/api/affiliate/apply', getUserMiddleware, async (req, res) => {
        try {
            const userId = req.dbUser.id;
    ...
    app.get('/api/admin/affiliate-stats', adminMiddleware, async (req, res) => {
        try {
            ...
            const totalEarnings = await db
                .select({ sum: sql `coalesce(sum(total_earnings), 0)::int` })
                .from(affiliateApplications);
            res.json({
    ...
    app.get('/api/charity-campaigns', async (req, res) => {
        try {
            const activeCampaigns = await db
                ...
                .where(and(eq(charityCampaigns.isActive, true), eq(charities.isActive, true)))
    ...
    app.get('/api/admin/charities', adminMiddleware, async (req, res) => {
        try {
    ...
    app.post('/api/admin/charities', adminMiddleware, async (req, res) => {
        try {
            const schema = z.object({
                name: z.string().min(1),
    ...
    app.patch('/api/admin/charities/:id', adminMiddleware, async (req, res) => {
        try {
            const { id } = req.params;
            const schema = z.object({
    ...
    app.delete('/api/admin/charities/:id', adminMiddleware, async (req, res) => {
        try {
            const { id } = req.params;
            await db
                .delete(charities)
    ...
    app.get('/api/admin/charity-campaigns', adminMiddleware, async (req, res) => {
        try {
    ...
    app.post('/api/admin/charity-campaigns', adminMiddleware, async (req, res) => {
        try {
            const schema = z.object({
    ...
    app.patch('/api/admin/charity-campaigns/:id', adminMiddleware, async (req, res) => {
        try {
            const { id } = req.params;
            const schema = z.object({
    ...
    app.get("/api/xp/stats", requireAuth, async (req, res) => {
        try {
            const userId = req.dbUser.id;
            ...
            const gameStats = await db
                ...
                xpEarned: sql `COALESCE(SUM(${userGames.xpEarned}), 0)`,
                matchesPlayed: sql `COALESCE(COUNT(${userGames.id}), 0)`
            })
                .from(userGames)
    ...
    app.post("/api/xp/sync", requireAuth, async (req, res) => {
        try {
            const userId = req.dbUser.id;
            const schema = z.object({
    ...
    app.get("/api/referrals/:code", async (req, res) => {
        try {
            ...
            const referralStats = await db
                ...
                activeReferrals: sql `COUNT(*) FILTER (WHERE ${referrals.status} = 'active')`,
                totalPointsEarned: sql `COALESCE(SUM(${referrals.pointsAwarded}), 0)`
            })
                .from(referrals)
    ...
    app.get("/api/admin/integrity/alerts", adminMiddleware, async (req, res) => {
        try {
            const { severity, limit = 50 } = req.query;
            let query = db
                .select({
            ...
            if (severity) {
                query = query.where(and(eq(fraudDetectionLogs.status, 'pending'), eq(fraudDetectionLogs.severity, severity)));
            }
    ...
    app.get("/api/admin/integrity/patterns", adminMiddleware, async (req, res) => {
        try {
            ...
            const patternStats = await db
                ...
                detectionType: fraudDetectionLogs.detectionType,
                count: sql `COUNT(*)`,
                avgRiskScore: sql `AVG(${fraudDetectionLogs.riskScore})`,
                highSeverityCount: sql `COUNT(*) FILTER (WHERE ${fraudDetectionLogs.severity} IN ('high', 'critical'))`
            })
                .from(fraudDetectionLogs)
            ...
            const recentPatterns = await db
                ...
                detectionData: fraudDetectionLogs.detectionData,
                count: sql `COUNT(*)`,
                latestOccurrence: sql `MAX(${fraudDetectionLogs.createdAt})`
            })
                .from(fraudDetectionLogs)
                .where(and(eq(fraudDetectionLogs.status, 'pending'), sql `${fraudDetectionLogs.severity} IN ('high', 'critical')`))
    ...
    app.post("/api/admin/integrity/resolve/:id", adminMiddleware, async (req, res) => {
        try {
            const { id } = req.params;
            const adminId = req.dbUser.id;
            const schema = z.object({
