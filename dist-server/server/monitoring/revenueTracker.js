...
export class RevenueTracker {
    ...
    async getSnapshot() {
        try {
            ...
            const [newSubsResult] = await db
                .select({ count: count() })
                .from(subscriptions)
                .where(and(eq(subscriptions.status, 'active'), gte(subscriptions.createdAt, today)));
            // Churned today (status changed to canceled/past_due)
            const [churnResult] = await db
                .select({ count: count() })
                .from(subscriptions)
                .where(and(sql `${subscriptions.status} IN ('canceled', 'past_due')`, gte(subscriptions.updatedAt, today)));
     ...
     */
    async checkMilestones(snapshot) {
