...
export async function verifyPayPalSubscription(subscriptionId) {
    ...
    try {
        ...
        const planTierMap = {};
        if (basicPlanId)
            planTierMap[basicPlanId] = "basic";
        else if (!isProduction)
            planTierMap["P-6A485619U8349492UNEK4RRA"] = "basic"; // Sandbox fallback
        if (proPlanId)
            planTierMap[proPlanId] = "pro";
        else if (!isProduction)
            planTierMap["P-7PE45456B7870481SNEK4TRY"] = "pro"; // Sandbox fallback
        if (elitePlanId)
            planTierMap[elitePlanId] = "elite";
        else if (!isProduction)
            planTierMap["P-369148416D044494CNEK4UDQ"] = "elite"; // Sandbox fallback
