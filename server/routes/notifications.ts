...
import { db } from "../db";
import { notifications, notificationCampaigns, users } from "@shared/schema";
...
router.post("/mark-read", async (req: any, res) => {
  if (!req.isAuthenticated()) return res.sendStatus(401);
...
router.post("/send", ensureAdmin, async (req: any, res) => {
...
router.post("/campaigns", ensureAdmin, async (req: any, res) => {
