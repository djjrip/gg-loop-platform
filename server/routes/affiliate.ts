...
router.get('/procurement/:redemptionId', async (req, res) => {
    try {
        ...
        const procurement = await handleRewardProcurement(redemptionId, rewardId as string);
...
router.get('/dashboard', async (req, res) => {
    try {
        // Last 7 days
        const last7Days = await getAffiliateRevenue(
            new Date(Date.now() - 7 * 24 * 60 * 60 * 1000),
            new Date()
        );

        // Last 30 days
        const last30Days = await getAffiliateRevenue(
            new Date(Date.now() - 30 * 24 * 60 * 60 * 1000),
            new Date()
        );

        // All time
        const allTime = await getAffiliateRevenue(
            new Date('2024-01-01'),
            new Date()
        );
