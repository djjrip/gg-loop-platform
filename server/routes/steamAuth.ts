...
export function setupSteamAuth(app: Express) {
    ...
    passport.use(new SteamStrategy({
        ...
        async (req: any, identifier: string, profile: any, done: any) => {
            try {
                ...
                if (!steamProfile._json.profileurl) steamProfile._json.profileurl = `https://steamcommunity.com/profiles/${profile.id}`;
                if (!steamProfile._json.avatar) steamProfile._json.avatar = null;
    ...
    app.get('/api/steam/status', async (req: any, res) => {
        if (!req.isAuthenticated()) {
            return res.status(401).json({ error: "Unauthorized" });
        }
