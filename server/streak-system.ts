...
export async function updateLoginStreak(userId: string) {
    ...
    if (lastLogin) {
        ...
        if (hoursSinceLastLogin >= 24 && hoursSinceLastLogin < 48) {
            ...
            // Milestone bonuses
            if (newStreak === 7) bonusPoints = 100;   // Week 1
            if (newStreak === 14) bonusPoints = 250;  // Week 2
            if (newStreak === 30) bonusPoints = 500;  // Month
            if (newStreak === 60) bonusPoints = 1000; // 2 months
            if (newStreak === 90) bonusPoints = 2000; // 3 months
        }
 ...
 */
export function applyStreakMultiplier(basePoints: number, streak: number): number {
