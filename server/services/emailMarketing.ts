...
export async function sendUpsellEmail(userId: string) {
  try {
    ...
    if (sub && sub.status === 'active') return; // Already subscribed
 ...
 */
export async function sendAbandonedCartEmail(userId: string, rewardId: string) {
  try {
    const [user] = await db.select().from(users).where(eq(users.id, userId)).limit(1);
    const [reward] = await db.select().from(rewards).where(eq(rewards.id, rewardId)).limit(1);

    if (!user || !user.email || !reward) return;
 ...
 */
export async function sendRedemptionConfirmation(userId: string, rewardId: string) {
  try {
    const [user] = await db.select().from(users).where(eq(users.id, userId)).limit(1);
    const [reward] = await db.select().from(rewards).where(eq(rewards.id, rewardId)).limit(1);
