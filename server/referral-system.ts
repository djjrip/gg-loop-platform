 ...
 */
export async function processReferralSignup(referrerId: string, newUserId: string) {
 ...
 */
export async function processReferralFirstGame(userId: string) {
 ...
 */
export async function processReferralRedemption(userId: string, pointsSpent: number) {
    const referral = await db.query.referrals.findFirst({
        where: eq(referrals.referredUserId, userId)
    });
...
async function awardPoints(userId: string, amount: number, type: string, sourceId: string) {
