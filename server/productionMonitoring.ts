...
import { sendApiKeyEmailAlert } from './emailAlerts';

interface MonitoringAlert {
    severity: 'critical' | 'high' | 'medium' | 'low';
    category: 'revenue' | 'security' | 'performance' | 'error';
    message: string;
    details?: any;
}
 ...
 */
async function sendAlert(alert: MonitoringAlert) {
    const emailPayload = {
        personalizations: [{
...
export async function trackRevenueEvent(event: {
    type: 'subscription_created' | 'subscription_cancelled' | 'payment_failed' | 'redemption';
    amount?: number;
    userId?: number;
    details?: any;
}) {
