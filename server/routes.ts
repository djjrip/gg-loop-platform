  ...
  app.post('/api/riot/link-account-DISABLED', getUserMiddleware, async (req: any, res) => {
    return res.status(410).json({
      ...
      migration: {
        ...
        step3: 'POST /api/riot/verification/verify-ownership with {game}'
      }
    });
  });

  // SECURE: Link Riot account with ownership verification
  const riotVerificationRouter = await import('./routes/riotVerification');
  app.use('/api/riot/verification', getUserMiddleware, riotVerificationRouter.default);

  //OLD INSECURE ENDPOINT - Returns error explaining the new secure flow
  app.post('/api/riot/link-account', getUserMiddleware, async (req: any, res) => {
    return res.status(410).json({
      error: 'ENDPOINT_DEPRECATED',
      message: 'This endpoint has been replaced with a secure 2-step verification process to prevent account takeover attacks.',
      migration: {
        step1: 'POST /api/riot/verification/request-verification',
        step2: 'Add verification code to in-game Riot ID',
        step3: 'POST /api/riot/verification/verify-ownership'
      }
    });
  });

  // Keeping old route structure for backward compatibility but with security
