 ...
 */
async function updateStreamerEngagement(email: string, eventType: string) {
    ...
    try {
        ...
        const streamer = data.streamers.find((s: any) => s.email === email);
 ...
 */
export async function getEmailMetrics(campaignType?: string): Promise<EmailMetrics> {
 ...
 */
export async function getTopEngaged(campaignType?: string) {
    const whereClause = campaignType ? `WHERE campaign_type = ?` : '';
    const params = campaignType ? [campaignType] : [];
